...
from multipath_bp.bound_op_map_multipath import bound_op_map
from multipath_bp.bound_ops_multipath import *
from multipath_bp.bounded_tensor import BoundedTensor, BoundedParameter
from multipath_bp.parse_graph import parse_module
from multipath_bp.perturbations_multipath import *
from multipath_bp.utils import *
from multipath_bp.adam_element_lr import AdamElementLR
...
class BoundedModule(nn.Module):
    ...
    def __init__(self, model, global_input, bound_opts={}, auto_batch_dim=True, device='auto',
        ...
        # TODO why do we need judge isinstance
    ...
    # Just remove elif '.' in name compared with overridden.
    ...
    # node_name_map is a dict between LiRPA model and pth model
    ...
    def forward(self, *x, final_node_name=None):
        ...
        while len(queue) > 0:
            ...
            if not hasattr(l, 'batch_dim'):
                ...
                try:
                    # Infer batch_dim from inp_batch_dim (batch_dims of pre-layer nodes).
        ...
        while len(queue) > 0:
            ...
            for name_next in l.output_name:
                ...
                if isinstance(l, BoundShape):
                    # TODO: what is the meaning of Shape nodes?
    ...
    # Line 372 - 622 builds LiRPA models from pth
    ...
    # Line 623 - 664 (init_slope method) initialize alpha
    ...
    # Line 954 - 1325 (get_optimized_bounds): main computation method
    def get_optimized_bounds(self, x=None, aux=None, C=None, IBP=False, forward=False, method='backward',
        ...
        # C is a specification provided by users. It is add to the last layer to construct users' verification targets
        if C is not None:
...
# Line 2242 - end: over-write DataParallel
