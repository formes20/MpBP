...
from multipath_bp.perturbations_multipath import Perturbation, PerturbationLpNorm, PerturbationSynonym, PerturbationL0Norm
from multipath_bp.utils import eyeC, OneHotC, LinearBound, user_data_dir, isnan, Patches, logger, Benchmarking, prod, batched_index_select, patchesToMatrix, check_padding
...
# Line 126 - End: LiRPA model implemented like BoundLinear/BoundActivation(Bound(nn.Module)).
class Bound(nn.Module):
    ...
    def bound_forward(self, dim_in, *x):
        r"""
        Function for forward mode bound propagation. Forward mode LiRPA computs a `LinearBound` instance
        representing the linear bound for each involved node.
    ...
    def bound_backward(self, last_lA, last_uA, *x):
        ...
        Args: 
            last_lA (Tensor): `A` matrix for lower bound computation propagated to this node.
            It can be `None` if lower bound is not needed.

            last_uA (Tensor): `A` matrix for upper bound computation propagated to this node.
    ...
    def infer_batch_dim(self, batch_size, *x):
        """
        Infer the which dimension the batch_size locate.
        batch_dim = -1 means not be infered yet.

        Args:
            batch_size:
            *x: (list) pre-layer nodes' batch_dim list.
    ...
    def broadcast_forward(dim_in, x, shape_res):
        ...
        Extend x's shape to be same as shape_res.
        """
        ...
        while len(shape_x) < len(shape_res):
            # if has no batch size, extend the batch size dimension.
            if not has_batch_size:
                ...
                # The shape of lw is (batch, input_dim, output_dim).
                # The shape of lb is (batch, output_dim).
    ...
    def get_bias(self, A, bias):
        """
        Standardize the shape of A and bias, for getting bias.
        """
        if A is None:
            return 0
        if not Benchmarking:
    ...
    def clamp_mutiply(A, pos, neg):
        """
        Allocate contiguous memory for clamp multiply
        """
        Apos = A.clamp(min=0)
        Aneg = A.clamp(max=0)
...
class BoundReshape(Bound):
    ...
    def bound_backward(self, last_lA, last_uA, x, shape):
        def _bound_oneside(A):
            if A is None:
                return None
            # A shape is (spec, batch, path, *node_shape)
            return A.reshape(A.shape[0], A.shape[1], A.shape[2], *self.input_shape[1:])
...
class BoundLinear(Bound):
    ...
    def bound_backward(self, last_lA, last_uA, *x):
        # The shape of last_lA is (final dim, batch size, output dim).
        ...
        if not self.is_input_perturbed(1) and (not has_bias or not self.is_input_perturbed(2)):
            ...
            # Normal condition.
                ...
                def _bound_oneside(last_A):
                    ...
                    sum_bias = (last_A.to(input_lb[2]).matmul(input_lb[2]) if has_bias else 0.0)
                    return next_A, sum_bias

                # The shape of lA_x is (final dim, batch size, new output dim).
                # The shape of lbias is (final dim, batch size)
...
class BoundConv(Bound):
    ...
    def bound_backward(self, last_lA, last_uA, *x):
        ...
        def _bound_oneside(last_A):
            ...
            if type(last_A) == torch.Tensor:
                ...
                next_A = F.conv_transpose2d(last_A.reshape(shape[0] * shape[1] * shape[2], *shape[3:]), weight, None,
                ...
                if self.has_bias:
                    sum_bias = (last_A.sum((4, 5)) * x[2].lower).sum(3)
                    # Now the shape of sum_bias is (spec, batch, path, output_channel)
                else:
                    sum_bias = 0
                return next_A, sum_bias
            elif type(last_A) == Patches:
    ...
    def bound_forward(self, dim_in, *x):
        ...
        w_shape = mid_w.shape
        b_shape = mid_b.shape
        shape_wconv = [w_shape[0] * w_shape[1] * w_shape[2]] + list(w_shape[3:])
        shape_bconv = [b_shape[0] * b_shape[1]] + list(b_shape[2:])
        deviation_w = F.conv2d(
            diff_w.reshape(shape_wconv), weight_abs, None, 
            self.stride, self.padding, self.dilation, self.groups)
        deviation_b = F.conv2d(
            diff_b.reshape(shape_bconv), weight_abs, None,
            self.stride, self.padding, self.dilation, self.groups)
        center_w = F.conv2d(
...
class BoundActivation(Bound):
    ...
    def _add_linear(self, mask, type, k, x0, y0):
        ...
        Args:
            mask: (tensor) mask for different types.
            type: linearize lower bound or upper bound.
            k:
            x0:
            y0:
    ...
    def bound_backward(self, last_lA, last_uA, x):
        ...
        def _bound_oneside(last_A, sign=-1):
            ...
            # If batch locates at dimension 0, then unsqueeze the 0th dimension of lw.
            if self.batch_dim == 0:
                if sign == -1:
                ...
                # Squeeze the bias to shape (batch, node biases)
                while _bias.ndim > 2:
                    _bias = torch.sum(_bias, dim=-1)
            # If batch dimension is not known, unsqueeze the first two dimensions of lw.
    ...
    # TODO: batch_dim may be different since
    def infer_batch_dim(self, batch_size, *x):
        return x[0]
...
class BoundRelu(BoundOptimizableActivation):
    ...
    def init_opt_parameters(self, start_nodes):
        ...
        Args:
            start_nodes: (dict of name:shape)
    ...
    def bound_relax(self, x):
        ...
        self._add_linear(mask=self.mask_pos, type='lower',
                         k=torch.ones_like(expanded_x_lower), x0=0, y0=0)
