...
def accuracy_zhengye():
    test_data = datasets.CIFAR10('/data/zhengye/LiRPA_new/examples/vision/data', train=False, download=False, transform=transforms.ToTensor())
    model = cifar10_conv()
    model.load_state_dict(torch.load('./cifar10_conv.pth', map_location=torch.device('cpu')))

    correct_num = 0
    for i in range(1000):
        # Adjust to model input shape!!!
        image = torch.tensor(test_data.data[i].reshape(1, 3, 32, 32))
